...
　区間への書き込み、区間からの読み出し
    ...
    auto seg = new SegTree!(long, 
            ...
            0
                ...
                // 一律だと都合が悪い場合は、最初に for(i; 0 .. n ) setValue(i, i + 1, xxxx) で設定すればよい
    )(0, n);
        // 0 以上 n 未満の値を扱う
    
    値の設定
    seg.setValue(l, r + 1, v);
        // l 以上 (r + 1) 未満からなる区間に値 v をセットする
    
    値の取得
    ans = seg.getValue(l, r + 1);
        // l 以上 (r + 1) 未満からなる区間の計算結果
...
*/
class SegTree(T, T delegate(T, T) update, T delegate(T, int, T) apply, T delegate(T, int, T, int) merge, T initial){
	int a, b;
	int width;
	bool hasValue;
	SegTree left, right;
    ...
    // 値の書き込み
	void setValue(T v){ setValue(a, b, v); }
	...
	string resultString(){
		string r = ((result == initial)? "ini": result.to!string);
		if(left && right){
